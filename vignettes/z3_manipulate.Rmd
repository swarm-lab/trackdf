...
[`tibble`]() or [`data.table`]() with a few extra bells and whistles. This means 
that you can manipulate a track table same way you would a data frame of any of 
these three classes. 
...
calles `rbind_track` to achieve the same purpose. 
...
```{r}
plot(y ~ x , data = t_dt1, type = "l", col = "red", xlim = range(t_dt$x), 
     lwd = 2, asp = 1)
lines(y ~ x , data = t_dt2, type = "l", col = "blue",  lwd = 2)
```
